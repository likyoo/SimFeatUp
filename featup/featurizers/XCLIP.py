...
from featup.featurizers.maskclip import xclip


class XCLIPFeaturizer(nn.Module):

    def __init__(self, model_type="ViT-B/16"):
        super().__init__()
        self.model, self.preprocess = xclip.load(
            model_type,
...
if __name__ == "__main__":
    ...
    model = XCLIPFeaturizer().cuda()

    results = model(transform(image).cuda().unsqueeze(0))

    print(xclip.available_models())
